    ...
    <div className='markdown-content prose prose-blue max-w-none dark:prose-invert prose-ul:list-disc prose-ol:list-decimal'>
      <ReactMarkdown
        ...
        components={{
          ...
          td: ({ node, ...props }) => (
            <td
              className='px-6 py-4 whitespace-normal text-sm text-gray-500 dark:text-gray-400 border-r border-gray-300 dark:border-gray-700 last:border-r-0 border-b border-gray-300 dark:border-gray-700'
              {...props}
            />
          ),
          // Custom rendering for table rows with bottom borders
          tr: ({ node, ...props }) => <tr className='bg-white dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800' {...props} />,
